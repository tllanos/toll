...
public class Generator implements Runnable{
	...
	public Generator(ArrayList<Thread> vSim, ArrayList<Intersection> intersections, ArrayList<Toll> tolls){
		d = new Dijkstra();
		this.vSim = vSim;
		int count = 0, last = 0;
	...
	public synchronized void run(){
		while(true){
			...
			double f = (hour / 24.0d);
			int quantity = Math.abs((int)(((Math.sin((2 * Math.PI) * f)) * 600.0d) + 0.5d));
			...
			veh = new Vehicle(id++, 1, d.initDijkstra(tolls, intersections, init, dest));
			t = new Thread(veh);
			vSim.add(t);
			t.start();
			try {
